...
const ProfitScreen = () => {
  ...
  useEffect(() => {
    axios
      .get(`${API_BASE_URL}/seller/profitByYear/${user._id}/${new Date().getFullYear()}`)
  ...
  const getRevenueByYear = async (inputYear) => {
    setYear(inputYear);
    axios
      .get(`${API_BASE_URL}/seller/profitByYear/${user._id}/${inputYear}`)
      .then(function (response) {
        setData(response.data.result);
      })
      .catch(function (error) {
  ...
  return (
    ...
    <View style={{ flexDirection: "row", alignItems: "center" }}>
      <Text style={{ margin: 10 }}>Lợi nhuận cửa hàng năm</Text>
      <TouchableOpacity
        style={{
          paddingHorizontal: 5,
